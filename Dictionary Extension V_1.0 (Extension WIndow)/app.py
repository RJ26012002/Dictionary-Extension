...
#         for word in words:
#             query = f"SELECT definition FROM words WHERE word = '{word}'"
#             cursor.execute(query)
#             result = cursor.fetchone()

#             if result:
#                 definitions[word] = result['definition']
#             else:
#                 definitions[word] = 'Definition not found'

#         # Add definition for the entered phrase
#         entered_phrase = ' '.join(words)
#         entered_query = f"SELECT definition FROM words WHERE word = '{entered_phrase}'"
#         cursor.execute(entered_query)
#         entered_result = cursor.fetchone()

#         if entered_result:
#             definitions[entered_phrase] = entered_result['definition']
#         else:
...
def get_definitions():
    ...
    try:
        ...
        for word in words:
            query = f"SELECT meaning FROM words WHERE input_word = '{word}'"
            cursor.execute(query)
            result = cursor.fetchone()
        ...
        # Add definition for the entered phrase
        entered_phrase = ' '.join(words)
        entered_query = f"SELECT meaning FROM words WHERE input_word = '{entered_phrase}'"
        cursor.execute(entered_query)
        entered_result = cursor.fetchone()
